...
"""
    The difference from AntiBiasTrainDataset above is that AntiBiasTestDataset do not pad sentence
"""
...
if __name__ == "__main__":
    ...
    dataset = AntiBiasDataset(
        "unprejudiced_dataset/train.txt", tokenizer, 50, 'cpu')
    dataloader = DataLoader(dataset, batch_size=20)
    for i_batch, sampled_batched in enumerate(dataloader):
        if i_batch < 3:
            print(sampled_batched.dtype)
            print(sampled_batched.size())
