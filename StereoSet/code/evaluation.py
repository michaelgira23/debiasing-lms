...
class ScoreEvaluator(object):
    def __init__(self, gold_file_path, predictions_file_path_or_object):
        """
        ...
        Args:
            ...
            - predictions_file_path_or_object : path, relative or absolute, to the predictions file. Alternatively, the JSON object directly.
        ...
        if isinstance(predictions_file_path_or_object, str):
            with open(predictions_file_path_or_object) as f:
                self.predictions = json.load(f)
        else:
            self.predictions = predictions_file_path_or_object
        ...
        results['overall'] = self.evaluate(
            self.intersentence_examples + self.intrasentence_examples)
...
def parse_file(gold_file, predictions_file):
    ...
    with open(output_file, "w+") as f:
        print("Saving results at", output_file)
...
if __name__ == "__main__":
    ...
    if args.predictions_dir is not None:
        print("args.predictions_dir is not None")
        predictions_dir = args.predictions_dir
        ...
        print("Evalutaing", args.predictions_file)
